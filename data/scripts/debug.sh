...
if [ "$DEBUG_MODE" = "true" ]; then
    ...
    if [ -z "$DEBUG_OPT" ]; then
        # Debug mode is enabled, but no debug options are set
        # for docker debug , here DEBUG_PORT is not used
        JAVA_OPTS="$JAVA_OPTS -agentlib:jdwp=transport=dt_socket,address=*:5005,server=y,suspend=$DEBUG_SUSPEND"
    else
